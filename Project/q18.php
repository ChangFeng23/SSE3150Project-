...
if ( isset($_POST['next'] ) ) {
    ...
    $sql = "INSERT INTO score (value, questions_id, section) VALUES (:value, 18, 1)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(':value' => $_POST['score']));
    if($_SESSION['a']=='3'){
        $sql = "UPDATE `3150project`.`score` SET `value` = '5' WHERE (`section` = '1');";
        $stmt = $pdo->prepare($sql);
        $stmt->execute();
    }
    ...
    $sql = "DELETE FROM score WHERE questions_id = 17";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
...
<form method="POST">
 <div id=containerFea class="container-fluid">
        ...
        <?php 
        $stmt = $pdo->query("SELECT knowledgeArea From questions WHERE questions_id=18");
        ...
        <?php 
        echo('<table border="1">'."\n");
        $stmt = $pdo->query("SELECT detail From question_detail WHERE questions_id=18");
        ...
        <?php 
        echo('<table border="1">'."\n");
        $stmt = $pdo->query("SELECT name From clarifications WHERE questions_id=18");
        while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
            echo "<tr><td>";
        ...
        <?php                    
        echo('<table border="1">'."\n");
        $stmt = $pdo->query("SELECT * From rating WHERE questions_id=18");        
        while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
