...
if ( isset($_POST['next'] ) ) {
    ...
    $sql = "INSERT INTO score (value, questions_id, section) VALUES (:value, 47, 6)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(':value' => $_POST['score']));
    header("Location: q48.php");

}else if (isset($_POST['pre'] )){
    
    $sql = "DELETE FROM score WHERE questions_id = 46";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();

    header("Location: q46.php");
    return;
}
?>
<!DOCTYPE html>
...
<div class="jumbotron text-center text-black">
			...
			<h1>Project Management Integration Risks (6 Questions)</h1>
			<h2>Question 47</h2>
...
<form method="POST">
 <div id=containerFea class="container-fluid">
        ...
        <div class="col-lg-2 col-md-2 col-sm-12" style="background-color:lavender;">
           ...
           <?php 
        $stmt = $pdo->query("SELECT knowledgeArea From questions WHERE questions_id=47");
        ...
        <div class="col-lg-3 col-md-3 col-sm-12"style="background-color:orange;">
			  <?php 
        echo('<table border="1">'."\n");
        $stmt = $pdo->query("SELECT detail From question_detail WHERE questions_id=47");
        ...
        <?php 
        echo('<table border="1">'."\n");
        $stmt = $pdo->query("SELECT name From clarifications WHERE questions_id=47");
        while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
            echo "<tr><td>";
        ...
        ?>
         ...
         <div class="col-lg-2 col-md-2 col-sm-12"style="background-color:orange;">
           ...
           <?php                    
        echo('<table border="1">'."\n");
        $stmt = $pdo->query("SELECT * From rating WHERE questions_id=47");        
        while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
