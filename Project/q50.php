...
if ( isset($_POST['next'] ) ) {
    ...
    $sql = "INSERT INTO score (value, questions_id, section) VALUES (:value, 50,7)";
    ...
    $sql = "DELETE FROM score WHERE questions_id = 49";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
...
<form method="POST">
 <div id=containerFea class="container-fluid">
       ...
       <?php 
        $stmt = $pdo->query("SELECT knowledgeArea From questions WHERE questions_id=50");
        ...
        <div class="col-lg-3 col-md-3 col-sm-12"style="background-color:orange;">
			...
			<?php 
        echo('<table border="1">'."\n");
        $stmt = $pdo->query("SELECT detail From question_detail WHERE questions_id=50");
       ...
       <?php 
        echo('<table border="1">'."\n");
        $stmt = $pdo->query("SELECT name From clarifications WHERE questions_id=50");
        while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
            echo "<tr><td>";
        ...
        <?php                    
        echo('<table border="1">'."\n");
        $stmt = $pdo->query("SELECT * From rating WHERE questions_id=50");        
        while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
