...
if ( isset($_POST['next'] ) ) {
    ...
    $sql = "INSERT INTO score (value, questions_id, section) VALUES (:value, 53,7)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(':value' => $_POST['score']));
    header("Location: q54.php");

}else if (isset($_POST['pre'] )){
    
    $sql = "DELETE FROM score WHERE questions_id = 52";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
...
<form method="POST">
 <div id=containerFea class="container-fluid">
       ...
       <?php 
        $stmt = $pdo->query("SELECT knowledgeArea From questions WHERE questions_id=53");
        ...
        <div class="col-lg-3 col-md-3 col-sm-12"style="background-color:orange;">
			...
			<?php 
        echo('<table border="1">'."\n");
        $stmt = $pdo->query("SELECT detail From question_detail WHERE questions_id=53");
       ...
       <?php 
        echo('<table border="1">'."\n");
        $stmt = $pdo->query("SELECT name From clarifications WHERE questions_id=53");
        while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
            echo "<tr><td>";
        ...
        <?php                    
        echo('<table border="1">'."\n");
        $stmt = $pdo->query("SELECT * From rating WHERE questions_id=53");        
        while ( $row = $stmt->fetch(PDO::FETCH_ASSOC) ) {
